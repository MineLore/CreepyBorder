...
public class ChunkyBorderUtil {
    ...
    public static double distToBorder(Location location, Shape shape) {
        ...
        if (shape instanceof AbstractPolygon polygon) {
            return distToBorder(pos, polygon);
        }
        else if (shape instanceof AbstractEllipse abstractEllipse) {
            return distToBorder(pos, abstractEllipse);
        }
        else throw new IllegalArgumentException("Shape " + shape.name() + " no support");
    }
